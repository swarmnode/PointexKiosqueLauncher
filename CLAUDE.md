...
- `PROVISIONING_REQUIRED` → `ProvisioningRequiredScreen` — first-run screen shown when the app is not yet Device Owner (`KioskPolicyManager.isDeviceOwner`) and no PIN is set; "Vérifier à nouveau" re-checks and proceeds to `PIN_SETUP` once provisioned. A "Configurer en mode kiosque limité" option proceeds anyway in a degraded mode for devices that can never become Device Owner: `startLockTask()` then falls back to plain screen pinning and all `KioskPolicyManager` policies stay no-ops. Two mitigations harden this mode: a "Définir un code de verrouillage" button opens `ACTION_SECURITY_SETTINGS` so the technician sets a device lock credential (escaping screen pinning then lands on the keyguard instead of the system), and choosing limited mode requests the default-launcher `ROLE_HOME` via `RoleManager` (Android 10+) so the Home button keeps returning to the kiosk.
...
- `PinRepository` — admin PIN (4 or 6 digits) stored via `SecurePrefs`; verification is rate-limited (5 consecutive failures → 30 s lockout, persisted so it survives a reboot).
