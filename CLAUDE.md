...
- `PROVISIONING_REQUIRED` → `ProvisioningRequiredScreen` — first-run screen shown when the app is not yet Device Owner (`KioskPolicyManager.isDeviceOwner`) and no PIN is set; "Vérifier à nouveau" re-checks and proceeds to `PIN_SETUP` once provisioned. A "Configurer en mode kiosque limité" option proceeds anyway in a degraded mode for devices that can never become Device Owner: `startLockTask()` then falls back to plain screen pinning and all `KioskPolicyManager` policies stay no-ops. Two mitigations harden this mode: a "Définir un code de verrouillage" button opens `ACTION_SECURITY_SETTINGS` so the technician sets a device lock credential (escaping screen pinning then lands on the keyguard instead of the system; the technician must also set "Verrouiller automatiquement" to "Jamais" so the code is not asked on every screen wake), and choosing limited mode requests the default-launcher `ROLE_HOME` via `RoleManager` (Android 10+) so the Home button keeps returning to the kiosk. The choice is persisted (`KioskModeRepository`) because granting the HOME role recreates the activity. Field-tested constraints of screen pinning (Sunmi L2s PRO, Android 12): launching an allowed app while pinned hits an "Attempted Lock Task Mode violation" — even same-task launches break on the target app's internal navigation — so `onAppClick` unpins first and the next kiosk resume re-pins (Android asks the user to confirm pinning each time; silent re-pin is Device Owner-only).
